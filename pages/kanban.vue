<template>
  <div id="app">
    <HeaderPage :isProject="true" :project="project"/>
    <div
        ...
        >
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              <div
                  class="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-center"
              >
                <h3
                    id="modal-title"
    ...
    <div class="flex justify-center">
      <div class="min-h-screen flex overflow-x-scroll py-12">
        ...
        <div class="h-auto">
          <button
              v-show="!displayNewColumnNameInput"
              class="relative inline-flex items-center justify-center p-0.5 mb-2 mr-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-600 to-blue-500 group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800"
              @click="displayNewColumnNameInput = true"
          >
            <span
    ...
    <div
        v-show="displayChat"
        class="fixed bottom-20 right-8 h-96 w-72 bg-gray-100 shadow-xl border border-black rounded-md"
    >
      <div class="w-full h-full flex flex-col-reverse">
        <div
            class="flex self-end items-center basis-1/6 justify-between w-full border-t border-gray-300"
        >
          <input
          ...
          <button type="submit" @click.stop="sendMessage">
            <svg
                class="w-5 h-5 mr-2 text-indigo-700 origin-center transform rotate-90"
                fill="currentColor"
                viewBox="0 0 20 20"
        ...
        <div class="flex basis-5/6 overflow-y-scroll">
          <ul class="space-y-2 mt-2 w-full">
            <li
                v-for="(msg, index) in project.messages"
                :key="msg._id"
                :class="
                msg.user._id === $store.state.user.id
                  ? 'justify-end'
                  : 'justify-start'
              "
            ...
            >
              ...
              >
                <span>
                  <small
                      v-show="
                      index !== 0 && project.messages.length > 1
                        ? msg.user._id !== project.messages[index - 1].user._id
                        : true
                    "
                      class="text-gray-500"
                  >
    ...
    <button
        class="fixed bottom-7 right-7 bg-white w-14 h-14 filter opacity-70 rounded-full shadow-lg"
        @click.stop="displayChat = !displayChat"
    >
      <i class="fas fa-message text-2xl text-indigo-700"></i>
    </button>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    socket.on("projectFromId", function (project) {
      ...
      scope.$set(scope.project, project);
  ...
  methods: {
    checkToken: function (token, id) {
      if (typeof token === "undefined") this.$router.replace("/project-list");
